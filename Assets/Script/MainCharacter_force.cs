...
public class MainCharacter_force : MonoBehaviour
{
    ...
    void Update()
    {
        // 物理方式移動(而不是單純位移)：設定向量速度 = 給他一個方向向量作為移動速度
        ...
        if (Input.GetKey(KeyCode.RightArrow))
        {
            ...
            moveDir.x = moveSpeed;
            Debug.Log("向右移動的速度：" + m_Rigidbody2D.velocity);
        ...
        if (Input.GetKey(KeyCode.LeftArrow))
        {
            // m_Rigidbody2D.velocity = new Vector2(-moveSpeed, 0);
            moveDir.x = -moveSpeed;
            Debug.Log("向左移動的速度：" + m_Rigidbody2D.velocity);
        ...
        {
            // 這幾種寫法都可以，都是給一個向量，下方是將此向量作為速度去給予(不計重力)
            // m_Rigidbody2D.velocity = new Vector2(0, jumpSpeed);

            // 而下方則是將此向量作為一種物理上的力，差別在force會與向下的重力效果一起計算
